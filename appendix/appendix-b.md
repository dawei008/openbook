...
10 个核心类型的精简速查。每个类型只列出最重要的 5-8 个字段，并说明「为什么需要这个字段」。完整定义请查阅对应源文件。
...
**设计要点**: 泛型 `Tool<Input, Output, P>` 提供端到端类型安全。`buildTool()` 辅助函数为未显式定义的方法填充 fail-closed 默认值（如 `isConcurrencySafe` 默认返回 `false`）。
...
| `contentReplacementState?` | `ContentReplacementState` | 工具结果 token 预算管理；fork 子 Agent 会克隆父级状态以共享 cache 决策 |

**设计要点**: 这个类型很大（40+ 字段），因为它是工具执行的唯一上下文通道。可选字段 (`?`) 表示仅在特定场景下存在（如 `agentId` 仅子 Agent 设置，`setToolJSX` 仅交互模式提供）。
...
| 核心字段 | 类型 | 为什么需要 |
|----------|------|-----------|
| `data` | `T` (泛型) | 工具的实际输出数据，类型由 `Tool<Input, Output>` 的 `Output` 参数决定 |
...
**设计要点**: `SystemMessage` 有 14 种子类型 (informational / API error / compact boundary / agent killed / ...)，但它们都是纯 UI 消息 -- `normalizeMessagesForAPI` 会在发送给 API 前全部剥离。

---

## B.5 AppState

**位置**: `state/AppStateStore.ts`

全局应用状态。外层用 `DeepImmutable` 包裹确保不可变性，通过 `useSyncExternalStore` 实现切片订阅。

| 核心字段 | 类型 | 为什么需要 |
|----------|------|-----------|
| `settings` | `SettingsJson` | 用户/项目/策略设置的合并结果 |
| `toolPermissionContext` | `ToolPermissionContext` | 权限上下文：当前模式、允许/拒绝规则、附加工作目录 |
| `tasks` | `{ [taskId]: TaskState }` | 所有活跃后台任务的统一管理 |
| `mcp` | 嵌套对象 | 集中管理 MCP 连接、工具、命令和资源 |
| `plugins` | 嵌套对象 | 插件系统状态 (已启用/已禁用/错误/安装状态) |
| `agentDefinitions` | `AgentDefinitionsResult` | 去重后的 Agent 定义列表 + 加载失败记录 |
| `speculation` | `SpeculationState` | 推测执行状态 (idle / active)，支持投机性预计算 |
...
**位置**: `types/hooks.ts`, `entrypoints/agentSdkTypes.ts`
...
| `HookCallback.callback` | `(input, toolUseID, abort, ...) => Promise<HookJSONOutput>` | Hook 的实际执行逻辑 |
| `HookCallback.timeout?` | `number` | 防止 Hook 无限阻塞 |
| `HookCallbackMatcher.matcher?` | `string` | 匹配条件，如 `"Bash(git *)"` 只对特定工具+参数触发 |
| `HookResult.permissionBehavior?` | `'allow' \| 'deny' \| 'ask' \| 'passthrough'` | Hook 可以直接做出权限决策，优先级最高 |
| `HookResult.updatedInput?` | `Record<string, unknown>` | Hook 可以修改工具的输入参数 |
| `HookResult.additionalContext?` | `string` | 注入到模型上下文的额外信息 |
...
| `scope` | `ConfigScope` | 配置来源 (local/user/project/enterprise/...) |

**连接状态** (`MCPServerConnection`): `connected` / `failed` / `needsAuth` / `pending` / `disabled`。`ConnectedMCPServer` 包含 `capabilities` (服务器能力声明) 和 `cleanup` (断开清理回调)。
...
| `TaskStateBase.id` | `string` | 带类型前缀的唯一 ID (如 `b7k2m9x4` 表示 bash 任务) |
| `TaskStateBase.outputFile` | `string` | 输出文件路径，用于进程间通信 |
| `Task.kill()` | `(taskId, setAppState) => Promise<void>` | 终止任务的统一接口 |

**ID 生成规则**: 前缀 (`b`/`a`/`r`/`t`/`w`/`m`/`d`) + 8 位 `[0-9a-z]` 随机字符。36^8 约 2.8 万亿种组合，足以抵抗暴力猜测。

---

## B.9 PermissionResult

**位置**: `types/permissions.ts`
...
| `decisionReason` | 判别联合 | 记录决策原因：命中规则 (`rule`) / 权限模式 (`mode`) / Hook (`hook`) / 分类器 (`classifier`) 等 11 种 |
...
**位置**: `tools/AgentTool/loadAgentsDir.ts`
