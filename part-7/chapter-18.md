...
回顾整个 MCP 子系统，最令人印象深刻的不是某个单一的技术决策，而是**各种"不完美"的协调**。传输层有六种（因为没有一种能覆盖所有场景）。认证有两种（因为个人和企业需求截然不同）。配置有七层（因为不同的利益相关者需要不同的控制粒度）。错误标准化函数要对付 Slack 的非标准行为。去重逻辑要处理代理 URL 的解包。每个"不完美"都是对真实世界复杂性的务实回应。一个"完美"的设计——只有一种传输、一种认证、一层配置——只能在白板上存在。
