...
**stdio** 是本地子进程通信。配置只需要 command 和 args，type 字段甚至是 optional 的——不填就默认 stdio，这是向后兼容的考虑。实际连接时有一个值得注意的细节：stderr 被设为 `'pipe'`。如果 MCP Server 的错误输出直接打到终端，会破坏 Claude Code 的 TUI 界面。stderr 还被监听并累积到 64MB 上限的缓冲区中，用于在连接失败时提供诊断信息。stdio 进程的启动还支持通过环境变量指定 shell 前缀命令——在容器环境中可以通过前缀命令（如 `docker exec`）包装 MCP Server 的启动。
...
已连接状态不只存储 client 实例，还保存了 Server 的 `capabilities`（支持哪些协议能力）、`serverInfo`（名称和版本）和 `instructions`（Server 自述信息）。这些元数据让 Claude Code 能够根据 Server 的声明做适配——比如只向支持 elicitation 能力的 Server 发送用户交互请求。
