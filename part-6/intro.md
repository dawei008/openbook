...
**Chapter 17: 记忆系统全景 -- 从文件发现到梦境整合。** 「记忆就是文件」——不需要向量数据库，不需要 embedding 服务，只需要 Markdown 文件和一套管理机制。五层 CLAUDE.md 的发现策略、四类自动记忆的提取触发、相关性检索的关键词匹配、Dream 整合的碎片清理。记忆的完整生命周期：发现 --> 注入 --> 提取 --> 检索 --> 整合。
