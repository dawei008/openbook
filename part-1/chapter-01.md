...
| LLM 的缺陷 | Harness 的补全 | Claude Code 的实现 |
|---|---|---|
| 没有手 | 工具系统 | 40+ 个 Tool（Bash、FileEdit、Grep...） |
| 没有眼 | 上下文注入 | 系统提示词 + CLAUDE.md + 环境感知 |
| 没有记忆 | 对话管理 | 消息历史维护 + 自动压缩 |
| 没有缰绳 | 权限守卫 | 每个工具调用前的 `checkPermissions` |

这个设计类比 CSS 的层叠模型可能更好理解：LLM 提供默认行为（生成文本），Harness 在上面叠加了一层又一层的能力增强和行为约束，最终组合出 Agent 的完整行为。

### 实现

翻开 Claude Code 的源码，你会发现它几乎不包含任何 LLM 相关的模型代码。没有训练、没有推理、没有权重文件。整个代码库的全部工作就是构建 Harness -- 这一点从目录结构就一目了然。LLM 被当作一个外部服务通过 API 调用，而 Harness 就是围绕这个 API 调用构建的**一整套 TypeScript 运行时系统**。
...
Claude Code 采用了一种注册表模式：所有工具实现同一接口，统一注册到工具列表中，由运行时根据 LLM 的意图调度执行。这种设计的好处是**工具可以无限扩展**，而核心循环不需要修改。

关键的设计决策是：每个工具**必须**实现权限检查，这不是可选项。

```typescript
// Tool.ts -- 工具接口的核心方法（简化）
type Tool = {
  name: string
  call(args, context): Promise<ToolResult>     // 执行
  checkPermissions(input, ctx): Promise<Result> // 权限检查（必须实现）
  isReadOnly(input): boolean                   // 是否只读
  inputSchema: ZodSchema                       // 输入校验
}
```

`inputSchema` 是 Zod Schema，同时承担运行时验证和 TypeScript 类型推断。这意味着 LLM 生成的参数在执行前必须通过严格校验 -- LLM 说"读取 `/etc/passwd`"，Schema 先验证参数格式，`checkPermissions` 再验证权限，都通过了 `call` 才执行。三道关卡，缺一不可。

### 实现

工具覆盖了软件开发的完整生命周期。从源码 `tools.ts` 中的 `getAllBaseTools()` 可以看到全貌：文件读写（FileRead、FileEdit、FileWrite）、命令执行（Bash）、代码搜索（Grep、Glob）、子 Agent 派发（Agent）、网络获取（WebFetch、WebSearch）等。总计超过 40 个工具，每个都是一个独立目录，包含实现、提示词和常量定义。
...
权限系统的设计哲学是**默认保守**。看 `buildTool()` 工厂函数提供的默认值就明白了：

```typescript
// Tool.ts 第 757-769 行
const TOOL_DEFAULTS = {
  isConcurrencySafe: (_input?) => false,   // 默认不可并行
...
- **工具调用结果** -- 每次工具执行的输入和输出
- **CLAUDE.md 内容** -- 项目级的自定义指令（类似 `.editorconfig` 对编辑器的作用）
...
Claude Code 的 Harness 提供了四种扩展机制，按侵入性从低到高排列：

- **MCP (Model Context Protocol)** -- 通过标准协议连接外部工具和数据源。MCP 服务器可以用任何语言编写，通过 stdio 或 SSE 与 Claude Code 通信。这是最推荐的扩展方式，因为它完全解耦 -- MCP 服务器对 Claude Code 的内部实现一无所知。
...
- **Plugins** -- 最深层的扩展点。插件可以注册新工具、新命令、新 MCP 服务器，甚至修改权限规则。Claude Code 有完整的插件生命周期管理：安装、启用、禁用、更新、市场分发。

这个四层扩展体系的设计哲学是**渐进式侵入**。大多数用户的需求可以通过 MCP 或 Skills 满足（零侵入），只有深度定制才需要 Hooks 或 Plugins。

### 实现

扩展机制的状态管理分散在两层。`bootstrap/state.ts` 中跟踪已注册的 Hooks 和已调用的 Skills：

```typescript
// bootstrap/state.ts
registeredHooks: Partial<Record<HookEvent, RegisteredHookMatcher[]>> | null
invokedSkills: Map<string, { skillName: string; content: string; agentId: string | null }>
```

`AppStateStore.ts` 中跟踪 MCP 连接和 Plugin 状态：
...
打开 `main.tsx` 的前 20 行，你看到的不是通常的 import 列表，而是一个精心编排的**并行启动序列**：
...
代码在 import 语句之间穿插了副作用调用 -- `startMdmRawRead()` 和 `startKeychainPrefetch()` 被插在模块加载之间。为什么？因为模块加载需要约 135ms，而这两个操作是 I/O 密集型的，可以利用模块加载的等待时间并行执行。

到第 209 行，所有模块加载完成，代码标记了一个性能检查点：
...
真正的初始化在 `entrypoints/init.ts` 中。这个函数被 `memoize` 包装确保只执行一次，内部的序列揭示了 Harness 需要管理多少基础设施：

```typescript
// entrypoints/init.ts 第 57-159 行（关键步骤）
export const init = memoize(async (): Promise<void> => {
  enableConfigs()                     // 加载配置系统
  applySafeConfigEnvironmentVariables() // 安全环境变量
  applyExtraCACertsFromConfig()       // TLS 证书（必须在首次握手前）
...
`preconnectAnthropicApi()` 也值得留意 -- 它在配置 CA 证书和代理之后，预先发起 TCP+TLS 握手（耗时 100-200ms），让这个时间与后续的 action handler 初始化重叠。这是一种经典的**延迟隐藏**技术。
...
`init.ts` 第 87 行的 `setupGracefulShutdown()` 看似平淡，实则至关重要。它注册了进程退出时的清理逻辑。再往下看，清理注册贯穿整个初始化流程：

```typescript
// entrypoints/init.ts 第 189 行
...
LSP 服务器管理器、子 Agent 创建的 Team 文件、遥测数据 -- 所有需要在退出时清理的资源都通过 `registerCleanup` 注册回调。注意 Team 清理使用了 lazy import（`await import(...)`），因为 swarm 代码在 feature gate 后面，大多数会话不会加载它 -- 清理代码也遵循同样的懒加载原则。

遥测系统的初始化更体现了对用户隐私的尊重：`initializeTelemetryAfterTrust()` 只在用户接受信任对话框之后才启动遥测收集。这是合规驱动的设计 -- 在用户同意之前，一个字节的遥测数据都不会被收集。
...
从工程角度看，Claude Code 的 Harness 远不是简单的胶水层。它包含了性能工程（并行预取、API 预连接）、安全工程（多层权限检查、默认保守策略）、可靠性工程（优雅退出、运行时时序感知）和隐私工程（信任后才采集遥测）。
...
2. `main.tsx` 在 import 语句之间穿插副作用调用来实现并行启动。这种写法违反了"import 应该无副作用"的常见约定。你认为这种权衡是否合理？在什么情况下启动时间的优化值得打破惯例？

3. Claude Code 的遥测初始化被延迟到用户接受信任对话框之后。如果你在设计一个开源的 Agent 框架，你会如何设计遥测的 opt-in/opt-out 机制？
