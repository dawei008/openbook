...
Claude Code 的解决方案是一套多层防御体系。在 `query.ts` 的主循环中，每次 API 调用前，消息要过一条预处理管线（第 365-467 行）：
...
可清理的工具被明确枚举（`services/compact/microCompact.ts` 第 41-50 行）：
...
Microcompact 还有一套基于时间的触发机制。`evaluateTimeBasedTrigger`（`microCompact.ts` 第 422-444 行）计算距离上一条 assistant 消息过了多久。如果超过阈值（说明用户离开了一会儿），服务端的 Prompt Cache 已经过期，反正要重新计费，不如趁机清理旧内容：
...
阈值计算在 `services/compact/autoCompact.ts`（第 62-91 行）：
...
这里有一个容易忽略的细节。`shouldAutoCompact`（第 160 行）接受一个 `snipTokensFreed` 参数：
...
注释引用了真实的数据分析："BQ 2026-03-10: 1,279 sessions had 50+ consecutive failures (up to 3,272), wasting ~250K API calls/day globally"。有些会话的上下文实在太大，摘要本身就超长，压缩注定失败。没有熔断器，系统会在每轮循环都发一次注定失败的压缩请求——每天浪费 25 万次 API 调用。三次失败后停止重试，简单粗暴但有效。
...
压缩 prompt 要求 LLM 生成包含 9 个部分的摘要（`services/compact/prompt.ts` 第 61 行起）。其中第 6 点特别重要："List ALL user messages that are not tool results"。这确保了用户的意图在压缩后不会丢失——即使所有工具输出都被浓缩了，用户说过的每一句话都被保留。
...
压缩 prompt 前面还有一段强硬的声明（`prompt.ts` 第 19-26 行）：
...
这是因为压缩使用 `maxTurns: 1`。如果 LLM 尝试调用工具（比如想读一下某个文件来写更好的摘要），这唯一的一次机会就浪费了，压缩直接失败。数据显示在 Sonnet 4.6 上，不加这个声明的工具调用率是 2.79%。

摘要还被格式化处理：`formatCompactSummary` 函数（`prompt.ts` 第 311-335 行）会剥掉 `<analysis>` 块。这个块是 LLM 的"草稿纸"——先分析再总结能提高摘要质量，但分析过程本身没有信息价值，留在上下文里只会浪费 token。
...
`truncateHeadForPTLRetry`（`services/compact/compact.ts` 第 243 行）实现了一种递归降级策略：从最老的消息开始丢弃，直到释放够空间。这是有损操作——被丢弃的内容不会出现在摘要里。但总比完全无法压缩好。

### 实现

算法将消息按 API 轮次分组，然后根据超出量计算需要丢弃多少组（第 260-271 行）：
...
两条路径：如果 API 错误消息里包含了精确的 token 超出量（"137500 tokens > 135000 maximum"），就精确计算要丢弃多少；否则粗暴地丢 20%。最多重试 3 次（`MAX_PTL_RETRIES`）。
...
Claude Code 的策略是：**主动重建最近访问的文件上下文**。压缩完成后，`createPostCompactFileAttachments` 会重新读取最近访问的文件，作为附件注入到压缩后的上下文中。

### 实现

重建的参数（`compact.ts` 第 122-124 行）：
...
压缩前还会剥离图片和 PDF（`stripImagesFromMessages`，第 145-200 行），替换为 `[image]` 和 `[document]` 标记。原因是双重的：图片不需要摘要（它们的语义已经在对话文本中被讨论过），而且图片可能导致压缩请求本身超过上下文限制。

压缩后的消息序列有严格的顺序（`buildPostCompactMessages`，第 330-338 行）：
...
`boundaryMarker` 是一个特殊的系统消息，标记压缩发生的位置。它的作用至关重要：`getMessagesAfterCompactBoundary` 函数（在 query.ts 第 365 行调用）确保 API 只看到最近一次压缩之后的消息。压缩前可能有数百条消息，压缩后被替换为这几条精心组织的消息。
...
扣留逻辑在流式循环中（`query.ts` 第 799-825 行）。多种可恢复错误（prompt-too-long、media-size-error、max-output-tokens）都用同一个 `withheld` 标志控制：
...
流式循环结束后，检查是否有被扣留的错误，并尝试反应式压缩（第 1119-1166 行）：
...
Stop Hooks 在这里被显式跳过（"Do NOT fall through to stop hooks"）。原因是：模型根本没有产出有效回复，Stop Hooks 没有什么可以评估的。如果让 Stop Hooks 运行，它们会注入额外的消息让循环继续——但上下文已经溢出了，继续只会制造死循环。
...
不可以。Claude Code 在 `query.ts` 中维护了一个跨压缩边界的 Token 预算追踪（第 291 行）：
...
`finalContextTokensFromLastResponse` 读取的是 API 返回的实际 input_tokens，不是估算值。这确保了预算追踪的精度。

注释里有一段关键解释："while context is uncompacted the server can see the full history and handles the countdown from {total} itself... After a compact, the server sees only the summary and would under-count spend"。换句话说：压缩前，服务端能看到完整历史，自己计算消耗；压缩后，服务端只看到摘要，无法知道之前花了多少，所以客户端必须通过 `remaining` 字段告诉它。
...
上下文窗口管理的本质是信息取舍：什么该记住，什么可以忘记。Claude Code 的答案有四个要点：
...
这套系统让 Claude Code 能在理论上有限的"记忆"中，维持长达数小时的复杂编程会话。不是魔法，是层层叠叠的工程防御。
