...
Claude Code 忠实地实现了这个模型，但加了一层关键的工程抽象：**把整个循环拆成两层**。外层查询引擎负责"会话生命周期"——状态管理、消息录制、预算控制；内层查询函数负责"单轮推理循环"——调 API、执行工具、决定是否继续。
...
终止条件的设计体现了一个原则：**多重保险**。不能只靠一个条件来停止循环，因为任何单一机制都可能失效。Claude Code 至少有七种终止方式，分布在查询函数和查询引擎两层。
...
另一个关键步骤是系统提示词的组装。Claude Code 的系统提示词不是一段静态文本，而是从三个来源并行拉取、分层组合的：
