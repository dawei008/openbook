...
Claude Code 提供了两个入口函数，定义在 `services/api/claude.ts`（第 709-780 行）：一个流式版本 `queryModelWithStreaming`（AsyncGenerator，逐步产出事件），一个非流式版本 `queryModelWithoutStreaming`（返回 Promise，用于不需要实时反馈的场景如压缩对话）。两者共享同一个底层 `queryModel` 实现，区别只是消费方式不同。

### 实现

在 QueryEngine 的消息分发器中（`QueryEngine.ts` 第 788-816 行），流式事件被逐一处理。最重要的是 Token 用量追踪：
...
流式传输还带来了一个巧妙的优化机会：前面第 3 章提到的 StreamingToolExecutor。当 LLM 流式输出中出现一个完整的 `tool_use` block，不必等整个响应结束就可以开始执行工具。query.ts 在流式循环中间就取回已完成的工具结果（第 848-862 行）：
...
Claude Code 的重试引擎 `withRetry`（`services/api/withRetry.ts` 第 170 行）采用了经典的**指数退避 + 随机抖动**，但在此基础上叠加了三层差异化策略：按错误类型、按请求来源、按运行模式。

最有趣的设计是：`withRetry` 本身也是一个 AsyncGenerator。重试等待期间，它不是沉默地 sleep，而是 `yield` 出系统消息，让用户看到"正在重试..."的提示。这解决了一个 UX 问题：用户不知道程序是挂了还是在等。

### 实现

指数退避的实现（`withRetry.ts` 第 530-548 行）：
...
Claude Code 把请求分成两类：**前台请求**（用户正在等结果的）和**后台请求**（摘要生成、标题生成、建议等）。后台请求在遇到 529 时直接放弃，因为用户感知不到它们失败，而重试只会加剧过载。

### 实现

前台请求的来源被明确枚举（`withRetry.ts` 第 62-82 行）：
...
没在这个集合里的来源——`prompt_suggestion`、`title_generation`、`session_memory` 等——直接失败。注释里写得很清楚："每次重试是 3-10 倍的网关放大，用户根本看不到这些失败"。

对于前台请求，连续 3 次 529 后会触发**模型降级**（`withRetry.ts` 第 327-352 行）：
...
`FallbackTriggeredError` 被 query.ts 的外层 try/catch 捕获，切换到备用模型重试。比如 Opus 过载了，降级到 Sonnet——不如原来聪明，但至少能用。

## 4.4 持久重试：无人值守的韧性

### 问题

在 CI/CD 或自动化场景下，Claude Code 可能无人值守运行几小时。遇到 API 限速怎么办？等多久？

### 思路

普通场景下，重试 10 次失败就放弃了。但对于无人值守场景（通过环境变量 `CLAUDE_CODE_UNATTENDED_RETRY` 开启），Claude Code 提供了一种"等到天荒地老"的模式。

这里有一个工程细节值得注意：长时间等待期间，宿主环境（比如容器编排系统）可能因为空闲而杀死进程。解决方案是每 30 秒发一次"心跳"。

### 实现

持久重试的参数设定（`withRetry.ts` 第 96-98 行）：

```typescript
const PERSISTENT_MAX_BACKOFF_MS = 5 * 60 * 1000     // 退避上限 5 分钟
const PERSISTENT_RESET_CAP_MS = 6 * 60 * 60 * 1000  // 最长等 6 小时
const HEARTBEAT_INTERVAL_MS = 30_000                  // 心跳 30 秒
```

长时间 sleep 被切成 30 秒的块（第 489-506 行）。每个块结束时 yield 一条系统消息，宿主看到标准输出有活动，就不会判定进程"僵死"。
...
429 限速还有一个特殊处理：如果服务端返回了 `anthropic-ratelimit-unified-reset` 头（告诉你什么时候限速结束），直接等到那个时间点，而不是傻乎乎地指数退避。窗口式限速（比如"5 小时限额"）的重置时间通常是精确的。
...
Claude Code 设计了一套三级恢复机制，核心思想是**先试最便宜的方案**。

第一级：也许根本不需要那么多输出空间。系统默认把 `max_output_tokens` 压到 8K（`utils/context.ts` 第 24 行），因为数据分析显示 p99 的输出只有约 5000 token。如果触碰了这个低限额，先升级到 64K 重试——一次干净的重试换取 8 倍的容量节约。
...
第二级恢复消息的措辞值得细看（`query.ts` 第 1224-1229 行）：
...
Claude Code 在三个维度管控 Token 预算，每个维度解决不同的问题：
...
输出限额的容量保留优化体现了数据驱动的思维（`utils/context.ts` 第 18-25 行）：

```typescript
// BQ p99 output = 4,911 tokens, so 32k/64k defaults over-reserve 8-16x
export const CAPPED_DEFAULT_MAX_TOKENS = 8_000
export const ESCALATED_MAX_TOKENS = 64_000
```

注释直接引用了 BigQuery 的分析结果。不到 1% 的请求会触碰 8K 限额，它们被升级到 64K——代价是一次额外的 API 调用，收益是 99% 的请求省了 8-16 倍的容量预留。

USD 预算控制在 QueryEngine 层面（第 972 行），每处理完一条消息就检查累计花费：
...
运行时模型切换的逻辑（`utils/model/model.ts` 第 145-167 行）：
...
export function getRuntimeMainLoopModel(params: {
  ...
  if (getUserSpecifiedModelSetting() === 'opusplan'
      && permissionMode === 'plan' && !exceeds200kTokens) {
...
另外，Haiku 在规划阶段也会被升级到 Sonnet。注释虽然没写但逻辑很清楚：Haiku 的规划能力不足以驾驭复杂任务的分解和编排。
...
`services/api/errors.ts` 中的 `getAssistantMessageFromError`（第 425 行起）是一个庞大的错误分类器。它的设计原则是：**每种分类对应一种可操作的指引**。不是告诉用户"429 错误"，而是告诉他"限速了，去 claude.ai/settings 开启额外用量"。
...
每个分支产出一个 `AssistantMessage`，带有 `apiError` 字段标识错误类型。这个字段被上层的恢复机制消费——比如 `prompt_too_long` 类型会触发 reactive compact，`max_output_tokens` 会触发截断恢复。错误分类不只是给人看的提示，更是给机器看的恢复信号。
...
Anthropic 的 Prompt Caching 机制允许标记消息中的"缓存断点"。被标记内容的 Token 在第一次请求时正常计费，后续请求如果前缀匹配，只收 1/10 的价格。

Claude Code 在两个地方设置缓存断点：系统提示词和最近几轮对话的消息末尾。这样，只要系统提示词和对话前缀不变，每轮循环只为新增的内容付全价。

缓存的 TTL 默认 5 分钟（`ephemeral`），但对符合条件的用户可以扩展到 1 小时（`services/api/claude.ts` 第 358-374 行）。TTL 的选择被"锁存"在会话启动时——防止 GrowthBook 配置在请求中途更新，导致同一会话内混用不同 TTL，反而破坏缓存。

### 实现

缓存控制的生成逻辑（`claude.ts` 第 358-374 行）：
...
`should1hCacheTTL` 内部做了两层判断：用户是否有资格（内部用户或订阅且未超量），查询来源是否匹配 GrowthBook 的白名单模式。两者都满足才开启 1h TTL。

这里有一个微妙的稳定性考量：资格和白名单在首次查询时被锁存到 bootstrap state，整个会话不再变化。注释里解释了原因："prevents mixed TTLs when GrowthBook's disk cache updates mid-request"——如果 TTL 在会话中途从 5min 变成 1h，新请求的 cache_control 和旧请求不同，服务端会认为是新的前缀，之前缓存的内容全部失效。
